...
[クイックスタート](#始め方) · [結果の場所](#結果を見る場所) · [プロンプト例](#プロンプト例) · [安全上の注意と回避策](#安全上の注意と回避策) · [トラブルシューティング](#トラブルシューティング) · [今後の目標](#今後の目標)
...
↓
issue comment が有効な場合、ActionAgent がマーク付きの結果コメントを投稿する
↓
AI が結果コメント、結果ファイル、ログ抜粋、関連ログを読み取り、あなたに報告する
```
...
デフォルトでは、ActionAgent はマーク付きの issue コメントを投稿し、タスクが実行された場合に切り詰めたログ抜粋を `.action-agent/result.json` にコミットし、完全なログは artifact として保持します。`run = true` のタスクがない場合は、リポジトリ状態を変更せずに終了します。完全なログをリポジトリに残す必要がある場合は、タスクで `[output].commit = true` を明示的に設定できます。
...
タスクが完了したら、AI には次の順序で結果を確認させてください。

1. 最新のマーク付き ActionAgent issue コメント
2. `.action-agent/result.json`
3. 完全なログを含む GitHub Actions artifact の `action-agent-output`
4. タスクが `[output].commit = true` を明示的に有効にした場合のみ、コミットされた出力ファイル

`run = true` のタスクがない場合、ActionAgent は新しい実行結果を書き込むべきではありません。
...
AI はタスクを書く前に、それらの名前を使えるように ActionAgent の実行環境を準備します。Secret の実際の値は不要であり、尋ねるべきではありません。
...
- 設定された issue に ActionAgent の結果コメントが投稿されているか
