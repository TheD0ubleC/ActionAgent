...
[Quick Start](#how-to-start) · [Results](#where-results-appear) · [Example Prompts](#example-prompts) · [Safety Guidance and Avoidance](#safety-guidance-and-avoidance) · [Troubleshooting](#troubleshooting) · [Future Goals](#future-goals)
...
↓
ActionAgent posts a marked result comment when issue comments are enabled
↓
AI reads the result comment, result file, log excerpt, and referenced logs, then reports back to you
```
...
By default, ActionAgent posts a marked issue comment, commits a bounded log excerpt in `.action-agent/result.json` when a task runs, and keeps full logs as artifacts. If there are no `run = true` tasks, it exits without changing repository state. Tasks can opt in to committing full logs with `[output].commit = true` when the complete repository-visible log is needed.
...
When a task finishes, ask the AI to read results in this order:

1. The latest marked ActionAgent issue comment
2. `.action-agent/result.json`
3. The `action-agent-output` GitHub Actions artifact, which contains full logs
4. A committed output file only when the task explicitly enabled `[output].commit = true`

If no task has `run = true`, ActionAgent should not write a new runtime result.
...
- Whether an ActionAgent result comment was posted on the configured issue
